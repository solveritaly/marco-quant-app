...
for i, ticker_name in enumerate(top_tickers):
    ...
    if d:
        with t_cols[i]:
            card_html = f'<div style="text-align:center; border:2px solid {d["Color"]}; border-radius:10px; padding:10px;">'
            card_html += f'<span style="color:{d["Color"]}; font-size:26px; font-weight:bold;">{d["Score"]}</span><br>'
            card_html += f'<b>{d["Symbol"]}</b><br>€{d["Prezzo"]}</div>'
            st.markdown(card_html, unsafe
